...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : dim. 14 avr. 2024 à 21:46
...
CREATE DEFINER=`afef`@`%` PROCEDURE `creerExemplaire` (IN `nbExemplaire` INT, IN `idDocument` VARCHAR(10), IN `dateAchatC` DATE)   BEGIN
    DECLARE nb INTEGER;
...
CREATE DEFINER=`afef`@`%` PROCEDURE `majDvd` (IN `idDvd` VARCHAR(10))   BEGIN
    DECLARE nb INTEGER ;
...
CREATE DEFINER=`afef`@`%` PROCEDURE `majLivre` (IN `idLivre` VARCHAR(10))   BEGIN
    DECLARE nb INTEGER ;
    SELECT COUNT(*) INTO nb
        FROM dvd
...
CREATE DEFINER=`afef`@`%` PROCEDURE `majLivresDvd` (IN `idLivreDvd` VARCHAR(10))   BEGIN
    DECLARE nb INTEGER ;
    SELECT COUNT(*) INTO nb
        FROM revue
...
CREATE DEFINER=`afef`@`%` PROCEDURE `majRevue` (IN `idrevue` VARCHAR(10))   BEGIN
    DECLARE nb INTEGER ;
    SELECT COUNT(*) INTO nb
        FROM livres_dvd
        WHERE  id =   idrevue ;
...
('0001', 'jean', 'paul', 'jeanpaul@mail.com', 'd833bc9900084e742ac71366b0f4f987beb70e8cd8b21a9baf999acc5079437f', '0001'),
('0002', 'fred', 'jean', 'fred@mail.com', 'd833bc9900084e742ac71366b0f4f987beb70e8cd8b21a9baf999acc5079437f', '0002'),
('0003', 'sam', 'robert', 'sam@mail.com', 'd833bc9900084e742ac71366b0f4f987beb70e8cd8b21a9baf999acc5079437f', '0003'),
('0004', 'caro', 'line', 'caro@mail.com', 'd833bc9900084e742ac71366b0f4f987beb70e8cd8b21a9baf999acc5079437f', '0004');
